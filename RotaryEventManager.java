			...
			if (m != null) {
				if (ConfigRegistry.ALLOWBAN.getState()) {
					if (m.isCrucial()) {
						...
						if (ev.isCanceled())
							RotaryCraft.logger.log("Something successfully cancelled the placement of "+m+". Unless this machine really need to be disabled, it is recommend you remove this placement ban.");
					}
				}
				else {
					if (ev.isCanceled())
