	...
	public boolean onItemUse(ItemStack is, EntityPlayer ep, World world, int x, int y, int z, int s, float par8, float par9, float par10)
	{
		...
		if (m == MachineRegistry.GASTANK) {
			...
			if (clicked.isEmpty())
				...
				ReikaChatHelper.writeString(String.format("%s contains %.3fB of %s of a capacity of %.3fB.", m.getName(), clicked.getLevel()/1000D, clicked.getFluid().getLocalizedName(), clicked.getCapacity()));
		}
