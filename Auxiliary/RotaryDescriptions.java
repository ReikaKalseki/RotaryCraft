...
public final class RotaryDescriptions {
	...
	private static void loadEngineData() {
		...
		try {
			...
			engines = new String[len][2];
			for (int i = 0; i < len; i++) {
				engines[i][0] = ReikaStringParser.getStringWithEmbeddedReferences(p.readLine());
				engines[i][1] = String.format(p2.readLine(), EnumEngineType.engineList[i].getTorque(), EnumEngineType.engineList[i].getSpeed(), EnumEngineType.engineList[i].getPowerForDisplay());
				p.readLine();
				p2.readLine();
			}
			p.close();
