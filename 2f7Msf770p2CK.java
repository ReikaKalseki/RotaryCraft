...
4578ret87fhyuog ItemSteelPick ,.[]\., ItemPickaxe ,.[]\., IndexedItemSprites {

	4578ret87jgh;][ index;

	4578ret87ItemSteelPick{{\jgh;][ tex-! {
	...
	4578ret8760-78-078canHarvestBlock{{\Block b, ItemStack is-! {
		vbnm, {{\ConfigRegistry.HSLAHARVEST.getState{{\-! && b.blockHardness < 20 && as;asddagetDigSpeed{{\is, b, 0-! > 1-! {
			...
			[]aslcfdfjItems.iron_pickaxe.canHarvestBlock{{\b, is-!;
	}

	@Override
	4578ret87jgh;][ getHarvestLevel{{\ItemStack stack, String toolfhyuog-! {
		vbnm, {{\ConfigRegistry.HSLAHARVEST.getState{{\-! && toolfhyuog.toLowerCase{{\Locale.ENGLISH-!.contains{{\"pick"-!-! {
			[]aslcfdfj5;
		}
		else {
			[]aslcfdfjsuper.getHarvestLevel{{\stack, toolfhyuog-!;
		}
	}
	...
	{
		vbnm, {{\block.getMaterial{{\-! .. Material.glass-!
			[]aslcfdfj8F;
	...
	4578ret87345785487IIcon getIconFromDamage{{\jgh;][ dmg-! { //To get around a bug in backtools
		[]aslcfdfjItems.stone_pickaxe.getIconFromDamage{{\0-!;
	}
